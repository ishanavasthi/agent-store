...
1. **Ingestion pipeline** — photos+captions → structured catalog (name, price, variants, stock, description) with per-field confidence. Low-confidence fields go to a merchant confirmation screen; nothing unconfirmed is ever published. Ground-truthed against hand labels; accuracy reported honestly. Hinglish/emoji captions ("₹499/- only, DM to order 🔥") are in-scope test cases, not accidents.
2. **Storefront core** — catalog + checkout engine, one codebase, two faces: per-merchant MCP server (`search_catalog`, `get_product`, `create_cart`, `checkout`, `get_order_status`) and ACP-flavored REST (discovery doc + catalog + checkout). Checkout creates real Razorpay **test-mode** Orders/Payment Links; webhooks confirm payment (test UPI `success@razorpay`).
...
Buyer (Claude via MCP) → `search_catalog` → cart negotiation → **Intent mandate** (want + budget, buyer-signed) → storefront returns **Cart mandate** (exact items + total + **price hash**, signed both sides) → buyer signs **Payment mandate** (references cart hash + idempotency key) → trust layer verifies signature, cap, idempotency, stock, price-hash → Razorpay Order + Payment Link → human approves the link (consent step) → webhook confirms → signed **machine-readable receipt** returned to the buyer → audit trail complete.
...
- **Mandates:** canonical-JSON payloads, detached Ed25519 signatures. Chain binding by hash: Cart mandate embeds `sha256(intentMandate)`; Payment mandate embeds `sha256(cartMandate)` + idempotency key. Cart pins a **price hash** — checkout fails closed if catalog price changed after signing.
- **Cap enforcement:** `sum(captured + pending payments for agent×merchant) + thisAmount ≤ cap`, checked in a transaction.
- **Idempotency:** payment-mandate idempotency key scoped to agent×merchant; replay returns the original result, never a second charge.
- **Refusals:** `{code, reason, recoverable, retryAfter?}` — structured, LLM-recoverable.
...
- **Protocol suite (the 30):** TS scenario runner hitting MCP/REST surfaces programmatically against the gateway stub — happy purchases, out-of-stock mid-cart, over-cap attempt, ambiguous query, price change between cart and payment, replayed payment mandate, decline + retry + fail-closed, oversell + refund, malformed mandate, unregistered agent. Deterministic, one command, CI-runnable.
