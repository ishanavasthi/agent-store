...
- **S1 — Razorpay test mode works end to end as assumed.** Test keys → create Order + Payment Link via Node SDK → approve link manually → webhook received on the deployed URL. → *Spike result: **Verified** (2026-08-24). Two complete purchases on real test rails against the deployed Railway endpoint: `pay_TTcZdNBYOrbnAa` ₹1,299.00 and a second at ₹2,598.00 (a different amount, so the paid-amount assertion is exercised rather than trivially satisfied). Both captured at Razorpay, both Orders flipped to `paid`, both audit chains complete with no missing steps. Three findings, all fixed: (1) `customer: {}` in the Payment Link payload is rejected outright by the live API (`faulty key: customer`) — the field must be absent, and the SDK's types wrongly mark it required, so **every** checkout against real rails failed until this was found; (2) linking Razorpay's gateway order id was a read-then-write race — Razorpay fires three webhooks at once, all three saw a null id and all three linked, producing three `gateway.order_linked` events for one real linking (`order.paid` was unaffected because its guard already sat in the WHERE clause); (3) migrations cannot run in Railway's build step — its build sandbox cannot open the Neon WebSocket (`ETIMEDOUT`) though the runtime network can, so they moved to `preDeployCommand`. Post-fix chain, confirmed on the second purchase: `order.created · payment_link_attempted · payment_link_issued · webhook_received ×3 · order_linked ×1 · order.paid ×1`.*
- **S2 — Remote MCP connectivity.** Minimal authless MCP server (Streamable HTTP) on Render; connect from the claude.ai free-account custom connector, and from Claude Code as fallback client. → *Spike result: **Likely** (2026-08-24, partial). The server half is **Verified**: an authless Streamable HTTP MCP server on `@modelcontextprotocol/sdk` 1.30, mounted on Express in stateless mode (`sessionIdGenerator: undefined`, fresh server + transport per request), answers `initialize`, `tools/list` and `tools/call` correctly over plain HTTP POST — driven from a local client, no credentials involved. Remote reachability is now also **Verified** (2026-08-24): deployed to Render at `https://agent-store-e4ka.onrender.com`, and `initialize` + `tools/list` succeed over public HTTPS through Render's proxy, returning all three tools. Only **claude.ai free-account connector attachment** remains unconfirmed — the last untested step, and the one that needs a human. Deploy note: the first two deploys failed at `npm ci` because the lockfile had been generated by npm 11 (Node 26 dev machine) in a shape npm 10 (Node 22 target) rejects; fixed by regenerating under npm 10.9.3 and pinning `engines.npm` + `packageManager`.*
- **S3 — Extraction quality floor.** Run gpt-5-mini on 5 realistic Hinglish captions+photos. <~70% name+price exact-match → retry with gpt-5; still below → K2 fires. → *Spike result: **Verified** (2026-08-26). **5/5 — 100% name+price exact-match on gpt-5-mini**, the default model, against hand labels written before any model ran and published at `fixtures/extraction-spike/` with the scoring rules (spec story 36). The runs themselves are committed at `fixtures/extraction-spike/runs/gpt-5-mini.json` and `runs/gpt-5.json` — every number in this paragraph is read off those two files, raw model output included, so none of it has to be taken on trust. The floor is ~70%, so **K2 does not fire**: photos+captions stay the primary ingestion format and WhatsApp price-list text stays secondary and schedule-gated. gpt-5 was run as a control and also scored 5/5, so the step-up exists with nothing left to buy. Per-field on gpt-5-mini: name 5/5, price 5/5, stock 5/5, variant labels 4/5; gpt-5 was 5/5 on all four. The dataset is deliberately trapped and the traps held: `MRP 2,999/- 👉 launch price sirf ₹1,899/-` extracted as ₹1,899 and not the MRP; the tote's "2 per customer max" did not become stock 2 against the 20 pieces the same caption states; the bucket-hat caption never says what the product is and the name came off the photo, while the `free shipping above 999` sitting next to its price was not read as one; and both captions whose correct stock answer is *unstated* — the cargo pants' "stock ready ✅" and the bucket hat, which states no count at all — came back null rather than inventing a quantity, which is the behaviour story 6 depends on. The single miss is instructive, not fatal: gpt-5-mini read the bucket hat's "one size fits all, beige" as the two variant labels `["one size fits all", "beige"]` where the answer is no variants at all (one implicit default Variant) — gpt-5 got it right. That is a prompt fix for T12 and touches neither name nor price. Story 42 is verified as stated, and stronger than "one line": the model is the `EXTRACTION_MODEL` environment variable, so the gpt-5 run above was `EXTRACTION_MODEL=gpt-5 npm run spike:extraction` against the identical commit — a config change with no source edit at all, which is what M4 will do in a deployment rather than a rebuild. Two caveats. **n=5, one run per model**: this clears a go/no-go gate comfortably, it is not a calibrated accuracy figure, and the reportable per-field accuracy is T11's job on the demo dataset. And **Assumption**, unverified: the per-field confidences the model self-reports (0.90–0.98 on name and price across the gpt-5-mini run) are not calibrated probabilities and were checked against nothing — M4's auto-publish threshold has to be tuned on observed behaviour rather than trusted from these numbers. For the record: 5 items in 22s on gpt-5-mini and 26s on gpt-5, served by the dated snapshots `gpt-5-mini-2025-08-07` and `gpt-5-2025-08-07`, via the Responses API with Structured Outputs and plain `fetch` (no SDK).*
