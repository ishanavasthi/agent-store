...
- **S3 — Extraction quality floor.** Run gpt-5-mini on 5 realistic Hinglish captions+photos. <~70% name+price exact-match → retry with gpt-5; still below → K2 fires. → *Spike result: **Verified** (2026-08-26). **5/5 — 100% name+price exact-match on gpt-5-mini**, the default model, against hand labels written before any model ran and published at `fixtures/extraction-spike/` with the scoring rules (spec story 36). The floor is ~70%, so **K2 does not fire**: photos+captions stay the primary ingestion format and WhatsApp price-list text stays secondary and schedule-gated. gpt-5 was run as a control and also scored 5/5, so the step-up exists with nothing left to buy. Per-field on gpt-5-mini: name 5/5, price 5/5, stock 5/5, variant labels 4/5. The dataset is deliberately trapped and the traps held: `MRP 2,999/- 👉 launch price sirf ₹1,899/-` extracted as ₹1,899 and not the MRP; the tote's "2 per customer max" did not become stock 2 against the stated 20; a caption that never names the product got its name off the photo; and both captions whose correct stock answer is *unstated* ("stock ready ✅", and the per-customer limit) came back null rather than inventing a quantity, which is the behaviour story 6 depends on. The single miss is instructive, not fatal: gpt-5-mini read "one size fits all, beige" as two variant labels where the answer is no variants at all (one implicit default Variant) — gpt-5 got it right. That is a prompt fix for T12 and touches neither name nor price. Story 42 is verified as stated: the gpt-5 run above came from editing one line — `EXTRACTION_MODEL` in `src/ingestion/extractionModel.ts`, `DEFAULT_EXTRACTION_MODEL` → `STEP_UP_EXTRACTION_MODEL` — and changing nothing else. Two caveats. **n=5, one run per model**: this clears a go/no-go gate comfortably, it is not a calibrated accuracy figure, and the reportable per-field accuracy is T11's job on the demo dataset. And **Assumption**, unverified: the per-field confidences the model self-reports (0.82–1.00 across this run) are not calibrated probabilities and were checked against nothing — M4's auto-publish threshold has to be tuned on observed behaviour rather than trusted from these numbers. For the record: 5 items in 35s on gpt-5-mini, 39s on gpt-5, via the Responses API with Structured Outputs and plain `fetch` (no SDK).*
